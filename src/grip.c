		   ...
		   gboolean local_mode,gboolean no_redirect)
{
  ...
  g_signal_connect(G_OBJECT(monitor), "volume-added", G_CALLBACK(GripVolumeAdded), (gpointer)&(ginfo->Disc.info));
  g_signal_connect(G_OBJECT(monitor), "volume-removed", G_CALLBACK(GripVolumeRemoved), (gpointer)&(ginfo->Disc.info));
